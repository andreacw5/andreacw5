    ...
    <v-row align="center" class="h-100" justify="center">
      <v-col class="d-flex justify-center" cols="12" lg="6">
        ...
        >
          ...
          <div class="d-flex align-center justify-space-between text-body-2 text-sm-h6 font-weight-regular mb-4">
          ...
          <v-alert
            v-if="error"
            class="mb-4"
            :title="$t('auth.reset.errors.invalidToken')"
            :text="$t('auth.reset.retry')"
            type="error"
            variant="tonal"
            border="start"
            border-color="error"
...
const submit = async () => {
  ...
  // Call the API to update the password with the sec token
  ...
  snackbarStore.setSnackbar(
    { snackbar: snackbarStore.snackbar },
    {
      ...
      color: 'success',
      title: t('auth.reset.notifications.success.title'),
      text: t('auth.reset.notifications.success.content'),
    }
  );
